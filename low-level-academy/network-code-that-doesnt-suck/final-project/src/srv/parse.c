...
int update_employee(int fd, struct dbheader_t *dbhdr, struct employee_t *employees) {
...
void output_file(int fd, struct dbheader_t *dbhdr, struct employee_t *employees) {
    ...
    dbhdr->magic = htonl(dbhdr->magic); // Host to network long -- to account for endianess
    ...
    dbhdr->version = ntohs(dbhdr->count);
...
int validate_db_header(int fd, struct dbheader_t **headerOut) {
    ...
    header->version = ntohs(header->version); // This is done to fix endianness to make it transformabl
...
int create_db_header(int fd, struct dbheader_t **headerOut) {
    struct dbheader_t *header = calloc(1, sizeof(struct dbheader_t)); // We are able to ask the heap to give us a
